...
func New(username, password, endpoint, urchinCacheHashKey string, redisEndpoints []string, redisPassword string, enableCluster bool) (Client, error) {
	b := &client{
		username:           username,
		password:           password,
		endpoint:           endpoint,
	...
	if b.username == "" || b.password == "" || b.endpoint == "" || b.urchinCacheHashKey == "" {
...
func (c *client) sendHttpRequest(ctx context.Context, httpMethod, httpPath string, jsonBody string, respData interface{}) error {
	httpUrl := c.endpoint + strings.TrimLeft(httpPath, "/")
	for {
		r := &Reply{}
...
func initClient() (Client, error) {
	client, err := New("xxx", "yyy", "eci.paracloud.com",
