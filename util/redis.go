...
/*
	online k8s: RedisClusterIP = []string{"dragonfly-redis-master.dragonfly-system.svc.cluster.local:6379"}
	dev:	RedisClusterIP = []string{"10.18.119.29:6379", "10.20.3.170:6379"}
*/

var RedisClusterIP = []string{"10.18.119.29:6379", "10.20.3.170:6379"}
...
func (s *RedisStorage) LPush(key string, value string) error {
	_, err := s.client.LPush(key, value).Result()
	if err != nil {
		return err
	}
...
func (s *RedisStorage) LLen(key string) (int64, error) {
	count, err := s.client.LLen(key).Result()
	if err != nil {
		return -1, err
	}
...
func (s *RedisStorage) LTrim(key string, start, stop int64) error {
	_, err := s.client.LTrim(key, start, stop).Result()
	if err != nil {
		return err
	}
...
func (s *RedisStorage) LRange(key string, start, stop int64) ([]string, error) {
	members, err := s.client.LRange(key, start, stop).Result()
	if err != nil {
		return []string{}, err
	}
